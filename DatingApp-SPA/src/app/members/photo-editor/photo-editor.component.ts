...
export class PhotoEditorComponent implements OnInit {
  ...
  initializeUploader() {
    ...
    // of the 'Access-Control-Allow-Origin' header in the response must not be the
    // wildcard '*' when the request's credentials mode is 'include'. The credentials
    ...
    this.uploader.onSuccessItem = (item, response, status, headers) => {
      if (response) {
        ...
        // checks if photo is 1st and changes to nav and member detail photo
        if (photo.isMain) {
  ...
  deletePhoto(id: number) {
    this.alertify.confirm('Are you sure you want to delete this photo?', () => {
      this.userService.deletePhoto(this.authService.decodeToken.nameid, id).subscribe(() => {
        ...
        this.alertify.success('Photos has benn deleted');
      }, error => {
